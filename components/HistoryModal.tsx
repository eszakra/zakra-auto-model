...
const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-6xl h-[85vh] border border-gray-200 bg-white shadow-2xl flex flex-col relative rounded-xl overflow-hidden">
                ...
                <div className="flex justify-between items-center border-b border-gray-200 p-4 bg-white">
                    ...
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-900 transition-colors p-1 hover:bg-gray-100 rounded-lg">
                        <X size={20} />
                    </button>
                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {generations.map((gen) => (
                                    <div
                                        ...
                                        className={`aspect-square border-2 cursor-pointer relative group overflow-hidden transition-all rounded-lg ${selectedGen?.id === gen.id ? 'border-reed-red ring-2 ring-reed-red/20' : 'border-gray-200 hover:border-gray-300'}`}
                                    >
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm p-2 transform translate-y-full group-hover:translate-y-0 transition-transform border-t border-gray-100">
                                            <div className="text-xs text-gray-900 font-semibold truncate">{gen.model_name}</div>
                        ...
                        {selectedGen ? (
                            <div className="flex flex-col h-full">
                                ...
                                <div className="flex-1 flex items-center justify-center p-8 overflow-hidden bg-gray-50">
                                    <img src={selectedGen.image_url} className="max-w-full max-h-full object-contain shadow-xl rounded-lg border border-gray-200" />
                                </div>

                                {/* METADATA FOOTER */}
                                <div className="h-auto min-h-[180px] border-t border-gray-200 bg-white p-6">
                                    <div className="flex gap-6">
                                        <div className="flex-1 space-y-4 overflow-y-auto">
                                            <div>
                                                <label className="text-xs text-gray-500 uppercase font-medium block mb-2">Prompt Used</label>
                                                <div className="text-xs text-gray-700 font-mono leading-relaxed bg-gray-100 p-3 border border-gray-200 rounded-lg max-h-40 overflow-y-auto">
                                                    {selectedGen.prompt ? (
                                                                        ...
                                                                        <div className="space-y-2">
                                                                            {parsed.contents?.[0]?.parts?.[0]?.text && (
                                                                                <div>
                                                                                    ...
                                                                                    <p className="mt-1 text-gray-600">{parsed.contents[0].parts[0].text}</p>
                                                                                </div>
                                                                            )}
                                                                            <details className="mt-2">
                                                                                <summary className="cursor-pointer text-gray-500 hover:text-gray-700">View Full JSON</summary>
                                        ...
                                        <div className="w-72 space-y-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <span className="text-sm text-gray-900 font-medium block">{selectedGen.model_name}</span>
                                                </div>
                                                <div>
                                                    <label className="text-xs text-gray-500 uppercase font-medium block mb-1">Date</label>
                                                    <span className="text-sm text-gray-900 font-medium block">{formatDate(selectedGen.created_at)}</span>
                                                </div>
                                                <div>
                                                    <label className="text-xs text-gray-500 uppercase font-medium block mb-1">Resolution</label>
                                                    <span className="text-sm text-gray-900 font-medium block">{selectedGen.resolution || 'AUTO'}</span>
                                                </div>
                                                <div>
                                                    <label className="text-xs text-gray-500 uppercase font-medium block mb-1">Aspect</label>
                                                    <span className="text-sm text-gray-900 font-medium block">{selectedGen.aspect_ratio || 'AUTO'}</span>
                                                </div>
                                            </div>
